...
class ClientContactDetail(BaseModel):
    ...
    postal_address_line_2 = models.CharField('Postal Addres 2', max_length=100, blank=True, null=True)
...
class EmploymentDetail(BaseModel):
    ...
    employment_date = models.DateField('Employment Date', auto_now=False, auto_now_add=False, default='2018-01-01')
    personnel_number = models.CharField('Personnel Number', max_length=50, blank=True, null=True)
    medical_aid = models.CharField('Medical Aid', max_length=5, choices=ch_medical_aid, default='no')
    retirement_fund_current_value = models.DecimalField('Retirement Fund Current Value', max_digits=20,
                                                        decimal_places=2, blank=True, null=True)
    group_life_cover = models.CharField('Group Life Cover', max_length=5, choices=ch_group_life_cover, default='no')
...
class RatesAndReturn(BaseModel):
    ...
    """
    client_id_fk = models.ForeignKey('ClientDetail', on_delete=models.CASCADE)
    inflation = models.DecimalField('Inflation', max_digits=5, decimal_places=2, blank=True, null=True)
    interest = models.DecimalField('Interest', max_digits=5, decimal_places=2, blank=True, null=True)
    return_rate = models.DecimalField('Return Rate', max_digits=5, decimal_places=2, blank=True, null=True)
...
class Dependent(BaseModel):
    ...
    relationship = models.CharField('Relationship', max_length=50, choices=ch_relationship, default='not specified')
